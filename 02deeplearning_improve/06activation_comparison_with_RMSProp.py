...
def train_neural_network(X_train, Y_train, X_test, Y_test, activation_type, loop=8000, learn_rate=0.4, h=4, beta=0.9, use_RMSProp=False):
    """训练神经网络并返回历史记录"""
    ...
    for i in range(loop):
        ...
        if use_RMSProp:
            ...
            # 更新参数
            w1 = w1 - (learn_rate / (np.sqrt(s_dw1) + 1e-8)) * dw1
            b1 = b1 - (learn_rate / (np.sqrt(s_db1) + 1e-8)) * db1
            w2 = w2 - (learn_rate / (np.sqrt(s_dw2) + 1e-8)) * dw2
            b2 = b2 - (learn_rate / (np.sqrt(s_db2) + 1e-8)) * db2
