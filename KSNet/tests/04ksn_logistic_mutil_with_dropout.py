...
dropout_linear1_relu = KSNet.KSDropout(dropout_rate=dropout_rate)
...
for epoch in range(loop_count):
    ...
    dropout_relu_a1 = dropout_linear1_relu.forward(relu_a1, training=True)
    a2 = linear2.forward(dropout_relu_a1)
    ...
    dropout_linear1_relu_dout = dropout_linear1_relu.backward(linear2_dout)
    linear1_relu_dout = linear1_relu.backward(dropout_linear1_relu_dout)
